            ...
            {siteConfig.links.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Label Container */}
                  <div 
                    className={`w-[32%] md:w-[28%] flex flex-col font-sans select-none transition-all duration-300 overflow-hidden ${
                      isLeftAligned ? "items-start text-left pl-1 md:pl-2" : "items-end text-right pr-1 md:pr-2"
                    }`}
                  >
                    <span 
                      className={`text-xs md:text-sm font-semibold tracking-wide transition-colors duration-500 truncate w-full ${
                        isHovered ? "text-zinc-950 font-bold" : "text-zinc-500"
                      }`}
                    >
                      {link.label}
                    </span>
                    {link.description && (
                      <span 
                        className={`text-[9px] md:text-[11px] leading-tight text-zinc-400 font-normal mt-0.5 transition-colors duration-500 truncate w-full ${
                          isHovered ? "text-zinc-600" : ""
                        }`}
                      >
                        {link.description}
                      </span>
                  ...
                  <a
                    ...
                    className={`w-[68%] md:w-[72%] block relative overflow-hidden rounded-[1.2rem] md:rounded-[1.7rem] border border-[#e5e4e7] bg-white aspect-[3.5/1] card-transition shadow-sm ${link.hoverClass} ${
                      isHovered ? "-translate-y-0.5 scale-[1.01]" : ""
                    }`}
                  >
                    {/* Character Eye Illustration */}
                    <img 
                      src={link.image} 
                      className={`w-full h-full object-cover object-center card-transition duration-700 ${
                        isHovered ? "scale-[1.04] brightness-[0.88] contrast-[1.02]" : "brightness-[0.98]"
                      }`}
                    ...
                    <div 
                      className={`absolute top-2.5 right-2.5 md:top-3.5 md:right-3.5 z-20 w-6.5 h-6.5 md:w-8 md:h-8 rounded-full bg-black/15 md:bg-white/20 backdrop-blur-md border border-white/25 flex items-center justify-center text-white/80 shadow-md card-transition ${
                        isHovered ? "bg-white/35 text-white scale-110 translate-x-0.5 -translate-y-0.5" : ""
                      }`}
                    >
                      <svg 
                        viewBox="0 0 24 24" 
                    ...
                    <div 
                      className={`absolute inset-0 bg-black/25 flex items-center justify-center transition-all duration-500 z-10 ${
                        isHovered ? "opacity-100" : "opacity-0 pointer-events-none"
                      }`}
                    >
                      <div 
                        className={`bg-white/15 backdrop-blur-lg border border-white/20 px-5 py-3 md:px-7 md:py-3.5 rounded-2xl text-white font-medium tracking-wide flex items-center gap-3.5 md:gap-4.5 shadow-xl transition-all duration-500 transform ${
